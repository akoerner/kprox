...
static constexpr int SK_BG    = 0x0410;
...
void AppSinkProx::_draw() {
    ...
    // curl tip
    disp.setTextColor(disp.color565(120, 160, 120), SK_BG);
    disp.drawString("curl example:", 4, y); y += 12;
    disp.setTextColor(disp.color565(160, 200, 160), SK_BG);
    String curl1 = "curl -X POST http://" + baseIP + "/api/sink";
    String curl2 = "  -H \"X-Auth: <hmac>\"";
    String curl3 = "  -d '{\"text\":\"hello\"}'";
    // Wrap each line at ~38 chars
    disp.drawString(curl1.substring(0, 36), 4, y); y += 11;
    if (curl1.length() > 36) { disp.drawString(curl1.substring(36), 4, y); y += 11; }
    disp.drawString(curl2, 4, y); y += 11;
    disp.drawString(curl3, 4, y); y += 13;
    ...
    disp.drawString("ENTER/BtnA flush sink to HID  ESC back", 2, H - SK_BOT_H + 2);
